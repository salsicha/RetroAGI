...
def draw_lstm(frame):
    ...
    print(f"Cell State (Long Term):  [Prev C] ───{pulse if frame==0 else '───'}───({f_valve})───{pulse if frame==1 else '───'}───({i_valve})───{pulse if frame==2 or frame==3 else '───'}───► [New C]")
    print("                                         │           ▲")
    print("                                   Forget Gate   Input Gate")
    print("                                         │           │")
    print(f"Inputs & Gates:          [Inputs x,h] ───┴───────────┴───────────({o_valve})──► [Output h_t]")
    print("===============================")
    print("\n[Frame Explainer]: " + [
