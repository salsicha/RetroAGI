...
class nTreeClus:
    ...
    def nTreeClus(self):
        ...
        # self.seg_mat['Class'] = self.seg_mat[cols_to_merge].any(axis=1)
        # Drop the original columns if needed
        # self.seg_mat.drop(cols_to_merge, axis=1, inplace=True)
        ...
        if (self.method in ["All","DT"]):
            ...
            fitted_tree                                  = dtree.fit(X=xtrain.astype(int),y=ytrain.astype(int))
        ...
        if (self.method in ["All","RF"]):
            ...
            fitted_forest                                  = forest.fit(X=xtrain.astype(int), y=ytrain.astype(int))
