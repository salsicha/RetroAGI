...
class MarioScenarioEnv:
    ...
    def __init__(self, width: int = 256, height: int = 240, world_width: int = None):
        self.width  = width
        self.height = height
    ...
    def step(self, action: int):
        ...
        if self.mario['on_ground'] and self.mario['_platform'] and self.mario['_platform']['moving']:
        ...
        if self.mario['x'] > self._max_x_reached:
            reward += (self.mario['x'] - self._max_x_reached) * 0.05
    ...
    def render(self) -> np.ndarray:
        ...
        for enemy in self.enemies:
            ...
            if enemy['dead']:
                ...
                pygame.draw.rect(self.screen, (160, 32, 240),
                                 pygame.Rect(sx, sy, enemy['w'], enemy['h']))
        ...
        pygame.draw.rect(self.screen, color,
                         pygame.Rect(msx, msy, self.mario['w'], self.mario['h']))
    ...
    def _build_info(self) -> dict:
        ...
        nc = _nearest(active_coins,   lambda c: c['rect'])          if active_coins   else {'dx': 1.0, 'dy': 1.0, 'dist': 1.0}
        ne = _nearest(active_enemies, lambda e: pygame.Rect(e['x'], e['y'], e['w'], e['h'])) if active_enemies else {'dx': 1.0, 'dy': 1.0, 'dist': 1.0}
        ...
        return {
            'mario':               {k: m[k] for k in ('x','y','vx','vy','on_ground','facing','skidding','coyote_frames','jump_buffer')},
    ...
    def _update_enemy(self, enemy: dict, platform_rects: list):
        ...
        if enemy['edge_aware'] and enemy['on_ground']:
            ...
            supported = any(
                r.left <= peek_x <= r.right and r.top <= feet_y <= r.bottom
...
def main():
    ...
    custom_scenario = {
        ...
        'enemies': [
            ...
            {'x': 272.0, 'y': 200.0, 'patrol_min': 270.0, 'patrol_max': 388.0,
             'speed': 2.0, 'edge_aware': True},
            [325, 150, 322, 438, 1.2],
            {'x': 522.0, 'y': 200.0, 'patrol_min': 520.0, 'patrol_max': 660.0,
             'speed': 1.8, 'edge_aware': True},
            [562, 150, 560, 598, 0.9],
        ],
        'goal': [730, 80, 16, 20],
    }
