...
class SinglePassLSTMConditionedAgentWorldModel(nn.Module):
    """
    Single-pass policy that conditions final action logits on LSTM dynamics.

    The actor transformer runs once. Its B/C controller outputs feed the LSTM
    world model, the predicted C stream is projected into A-stream context, and
    a final action head consumes the original A hidden state plus that context.
    The returned tuple matches the trainer-facing baseline shape, but no second
    transformer pass is run for critic feedback.
    """
    ...
    def __init__(
        ...
        ratio_ab = seq_len_b // seq_len_a
        self.ratio_bc = ratio_bc
        self.world_context_scale = float(world_context_scale)
        self.agent = HierarchicalAdaptiveModel(
            vocab_size,
            d_model=d_model,
            controller_schedule=controller_schedule,
        ...
        self.world_model = WorldModel(ratio_bc=ratio_bc)
        self.critic = Critic(seq_len_c, seq_len_a, d_model)
        self.world_conditioned_action_head = nn.Linear(d_model, vocab_size)
        self.world_conditioned_action_head.load_state_dict(self.agent.fc_out_A.state_dict())
        self.motor_controller = MotorPrimitiveController(
            ratio_ab=ratio_ab,
            ratio_bc=ratio_bc,
        )
    ...
    def forward(
        ...
        *,
        ...
        logits_a1, actions1, w_1, b_1, hidden_a = self.agent(
            src_A,
            src_B,
            src_C,
            criticism=None,
        ...
        world_context = self.critic(next_state_pred)
        if not critic_feedback_enabled:
            world_context = torch.zeros_like(world_context)
        conditioned_hidden = hidden_a + self.world_context_scale * world_context.to(
            device=hidden_a.device,
            dtype=hidden_a.dtype,
        )
        logits_final = self.world_conditioned_action_head(conditioned_hidden)
        actions_final = self.agent.controller(src_C, w_1, b_1)
        self.last_motor_primitives = self.motor_controller(
            logits_final,
            w_1,
            b_1,
