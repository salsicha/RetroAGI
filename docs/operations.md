...
| Play Command | `retroagi play --game smb --stage full --checkpoint data/full_smb/policy.pth --steps 1000 --render --fps 30`. Use `--sample --temperature 0.75` for stochastic action sampling, `--no-render` for headless playback, `--pause-at-start` for terminal-controlled stepping, `--no-reset-on-done` to stop after the first terminal episode, and `--record --record-dir artifacts/full_smb/recordings --recording-path artifacts/full_smb/play_manifest.npz` to preserve playback artifacts. Terminal controls are `p` for pause/resume, `r` for reset, and `q` for quit when stdin is interactive. |
