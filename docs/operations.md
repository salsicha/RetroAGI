...
| Training Command | `retroagi train --game smb --stage full --init-checkpoint data/full_smb/transferred_policy.pth --full-smb-vision-checkpoint data/vit/full_smb_vit.pth --perception-mode freeze --updates-per-epoch 1 --rollout-steps 64 --checkpoint data/full_smb/policy.pth --log-path artifacts/full_smb/train.jsonl`. |
